...
smoke = '''name: Pages browser smoke test
  ...
  browser-smoke:
    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          node-version: 22
      - name: Wait for deployed v5 Pages
        run: |
          set -e
          URL='https://temesotejam.github.io/reaction-wheel-rocking-sim/'
          for i in $(seq 1 40); do
            html=$(curl -fsSL "$URL" || true)
            if echo "$html" | grep -q 'app.js?v=20260903-5'; then exit 0; fi
            sleep 5
          done
          exit 1
      ...
      - name: Exercise uncapped-Q controller in Chromium
        run: |
          node <<'NODE'
          ...
          NODE
'''
Path('.github/workflows/pages-smoke.yml').write_text(smoke, encoding='utf-8')
